...
###############################################################################
def slice_im(image_path, out_name, out_dir, sliceHeight=416, sliceWidth=416,
    ...
    try:
        ...
        # img_cv2 = cv2.imread(image_path, cv2.IMREAD_LOAD_GDAL)
        image0 = cv2.imread(image_path, cv2.IMREAD_LOAD_GDAL)
    ...
    for x0 in range(0, image0.shape[1], dx):  # sliceHeight):
        for y0 in range(0, image0.shape[0], dy):  # sliceWidth):
            n_ims += 1
